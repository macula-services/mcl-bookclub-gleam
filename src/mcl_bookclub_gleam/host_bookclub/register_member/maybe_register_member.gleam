...
pub fn handle_from_map(state: member_state.MemberState, payload: Payload) -> desk.DeskResult {
...
pub fn handle(state: member_state.MemberState, command: register_member_v1.RegisterMember) -> desk.DeskResult {
...
fn events(command: register_member_v1.RegisterMember) -> desk.DeskResult {
  case member_registered_v1.new(dict.from_list([
    #(atom("member_id"), dynamic.string(register_member_v1.get_member_id(command))),
    #(atom("club_id"), dynamic.string(register_member_v1.get_club_id(command))),
    #(atom("name"), dynamic.string(register_member_v1.get_name(command))),
    #(atom("club_name"), dynamic.string(register_member_v1.get_club_name(command))),
  ])) {
...
pub fn dispatch(command: register_member_v1.RegisterMember) -> desk.DispatchResult {
  case register_member_v1.validate(command) {
    Ok(_) ->
