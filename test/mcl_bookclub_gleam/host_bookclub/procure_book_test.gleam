...
/// facts.
fn book(club_id: String) -> Result(procure_book_v1.ProcureBook, dynamic.Dynamic) {
  procure_book_v1.new(dict.from_list([
...
pub fn a_book_is_procured_on_its_own_stream_test() {
  test_support.run(fn() {
    ...
    let assert Ok(#(0, [event])) =
      procure_book_api.handle(dict.from_list([
...
pub fn the_state_folds_both_events_test() {
  let state =
    book_state.new("b1")
    |> book_state.apply_event(dict.from_list([
  ...
  let state2 =
    book_state.apply_event(state, dict.from_list([
